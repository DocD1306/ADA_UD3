...
public class CabinetHibernateDao extends GenericHibernateDao<Cabinet, Long> implements CabinetDao {
    ...
    public List<ActiveCabinetsByGenre> activeCabinetsByGenre(Session session, String genre) {
        ...
        String hql = """
                SELECT new dto2.ActiveCabinetsByGenre(c, g.genre) FROM Cabinet c JOIN c.game g 
                WHERE c.status = 'ACTIVE' AND g.genre = :genre""";
    ...
    public List<Cabinet> cabinetsWithoutGamesSince(Session session, LocalDateTime rangeDate) {
        ...
        Root<Cabinet> root = cq.from(Cabinet.class);

        Subquery<Cabinet> subquery = cq.subquery(Cabinet.class);
        Root<Match> subRoot = subquery.from(Match.class);


        subquery.select(root).where(
            cb.and(
                    cb.equal(subRoot.get("cabinet"), root),
                    cb.greaterThan(subRoot.get("endAt"), rangeDate)
            )
        );
